...
```
> amazon_price & ebay price fields are VARCHAR() instead of INTEGER because the price format from Amazon and eBay are quite difficult to deal with if we want to cast them to INTEGER data types, this will be changed in the future for obviuos reasons (operate), by the moment we only want to show them
