...
```
> amazon_price & ebay price fields are VARCHAR() instead of DECIMAL(10,2) because the price format from Amazon and eBay are quite difficult to deal with if we want to cast them to INTEGER data types, this will be changed in the future for obvious reasons (to operate), by the moment we only want to show them
