...
The idea the following one, every time we execute the program a comparison functionality between the old products (the ones that are in the DB) and the new products (the same product in the DB) to check any price update of same product, namely, since the moment was their price consulted (when it was stored in the DB) and now (when we execute the program)
